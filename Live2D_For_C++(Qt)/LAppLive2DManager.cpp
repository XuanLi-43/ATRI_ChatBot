...
{
    // 遍历ResourcesPath中的所有文件夹，定义包含模型的文件夹。
    // 文件夹存在但找不到同名的.model3.json文件时，则不将其包含在列表中。
    // 由于部分字符接收不到，因此使用宽字符接收并转换为UTF8格式存储。
    ...
    while (_wfindnext(fh, &fdata) == 0)
    {
        if ((fdata.attrib & _A_SUBDIR) && wcscmp(fdata.name, L"..") != 0)
        {
            ...
            // 检查该文件夹中是否存在与文件夹同名的.model3.json文件
...
void LAppLive2DManager::StartLipSync(const Csm::csmString& filePath) {
